...
async function safeQuery(callback, label, errors) {
  const sql = getSql()
  if (!sql) {
    errors.push("Database connection is missing.")
    return null
  }
...
export async function getAdminDashboardData() {
  ...
  const [siteSettings, homeSections, projects, certifications, researchItems, achievements, educationItems, socialLinks, skills, experienceItems] = await Promise.all([
    safeQuery((sql) => sql`select * from site_settings order by updated_at desc limit 1`, "site settings", loadErrors),
    safeQuery((sql) => sql`select * from home_sections order by sort_order asc, updated_at desc`, "home sections", loadErrors),
    safeQuery((sql) => sql`
      select
        p.*,
        coalesce(
    ...
    `, "projects", loadErrors),
    safeQuery((sql) => sql`select * from certifications order by is_featured desc, sort_order asc, issue_date desc nulls last`, "certifications", loadErrors),
    safeQuery((sql) => sql`
      select
        r.*,
    ...
    `, "achievements", loadErrors),
    safeQuery((sql) => sql`select * from education order by sort_order asc, start_year desc nulls last`, "education", loadErrors),
    safeQuery((sql) => sql`select * from social_links order by sort_order asc, platform asc`, "social links", loadErrors),
    safeQuery((sql) => sql`select * from skills order by category asc, proficiency_bucket asc, sort_order asc, name asc`, "skills", loadErrors),
    safeQuery((sql) => sql`
      select
        e.*,
        coalesce(
