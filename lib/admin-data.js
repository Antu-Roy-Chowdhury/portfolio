...
export async function getAdminDashboardData() {
  const [siteSettings, projects, certifications, researchItems, achievements, educationItems, socialLinks, skills, experienceItems] = await Promise.all([
    ...
    safeQuery((sql) => sql`select * from skills order by category asc, sort_order asc, name asc`),
    safeQuery((sql) => sql`
      select
        e.*,
        coalesce(
