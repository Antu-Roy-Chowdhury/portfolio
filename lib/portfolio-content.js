...
export async function getAchievementItems(placement = "research") {
  noStore()
  ...
  if (!rows?.length) return placement === "research" ? fallbackAchievements : []

  const visibilityKey = {
    home: "show_on_home",
    projects: "show_on_projects",
    research: "show_on_research",
  }[placement]

  const visibleRows = visibilityKey ? rows.filter((row) => row[visibilityKey]) : rows
...
export async function getHomeContent() {
  ...
  const [siteChrome, projects, researchItems, experiences, skills, sections, researchMetrics, recognitionItems] = await Promise.all([
    getSiteChrome(),
    getProjects(),
    getResearchItems(),
    getExperienceItems(),
  ...
  return {
    ...
    highlightStats: [
      ...
      { value: `${recognitionItems.length}`, label: "Recognitions & awards" },
    ],
    researchMetrics: [
      { label: "Research Works", value: researchMetrics?.totalPublications || "0" },
      { label: "Total Citations", value: researchMetrics?.totalCitations || "0" },
      { label: "h-index", value: researchMetrics?.hIndex || "0" },
      { label: "Research Interest", value: researchMetrics?.researchInterestScore || "0" },
    ],
    recognitionItems,
    ...
    featuredResearch: researchItems.filter((item) => item.featured).slice(0, 3),
